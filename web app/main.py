...
from Preprocess_Gaussian_Blur import load_color
...
def main():
    ...
    temp_output_file = os.path.join(image_folder, "z.jpg")
    ...
    if uploaded_file is not None:
        ...
        if st.button("Preprocess"):
            ...
            temp_file1 = os.path.join(image_folder, "x.jpg")
            temp_file2 = os.path.join(image_folder, "y.jpg")
            # temp_output_file = os.path.join(image_folder, "z.jpg")
            cv2.imwrite(temp_file1, fundus1)
            Clahe = CLAHE(temp_file1)
            cv2.imwrite(temp_file2, Clahe)
            img2 = load_color(temp_file2, sigmaX=10)
            cv2.imwrite(temp_output_file, img2)

            st.image(img2, caption="Preprocessed Image", use_column_width=True)

        if st.button("Predict"):
            st.text("Predicting...")

            # Load the preprocessed image
            preprocessed_img = cv2.imread(temp_output_file)
            ...
            ensemble_model.compile(optimizer=tf.keras.optimizers.Adam(), loss="categorical_crossentropy",
                                   metrics=["accuracy"])
